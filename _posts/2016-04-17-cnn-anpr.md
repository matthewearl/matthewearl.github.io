...
be present in real world images, as well as account for actual sensor noise.
...
topology used than the Google paper.
...
probability values at each "pixel".
...
Whoops, it looks like the *R* has been misread as a *P*. Here's the window from
the above image which gives the maximum presence response:
