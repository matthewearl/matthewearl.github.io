...
OpenCV and NumPy. Source code [is available here](https://github.com/
...
[gen.py](https://github.com/matthewearl/deep-anpr/blob/master/gen.py).
...
train.py)) takes about 6 hours using a nVidia GTX 970,
...
{% include img.html src="/assets/cnn-anpr/out-bad.jpg" alt="Out Bad" %}

<sup>[Image credit](#image_credits)</sup>

Whoops. Here's the window from the above image which gives the maximum presence
response:

{% include img.html src="/assets/cnn-anpr/bad-window.png" alt="Out Bad" %}
...
{% include img.html src="/assets/cnn-anpr/R.png" alt="R" %}

Note how the leg of the R is at a different angle to the leg of the R in the
input image.  To test this hypothesis I modified the image in GIMP to more
closely resemble to training font:

{% include img.html src="/assets/cnn-anpr/gimp-anim.gif" alt="GIMP Anim" %}

And sure enough, the detector now gets the correct result:

{% include img.html src="/assets/cnn-anpr/out-better.jpg" alt="Out Better" %}
...
possible to build a ANPR system without importing any domain-specific
...
suspect this approach would work here, however I've not bothered to implement
it for this project.
...
varied fonts. I could make my training data more varied by drawing from a
selection of fonts. It's not clear how fonts I would need for this approach to
be successful.

The slowness (3)) is a killer for many applications: A modestly sized input
image takes several seconds to process on a reasonably powerful GPU. I don't
...
en.wikipedia.org/wiki/Histogram_of_oriented_gradients), or just a simpler
neural net.
...
corresponding with 3 corners of the plate) looks interesting. Perhaps it could
even be coerced into reading the numbers during the final stage of the cascade?
